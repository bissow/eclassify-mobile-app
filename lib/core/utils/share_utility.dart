...
typedef _ShareAction = ({
  IconData icon,
  String label,
  Future<void> Function(BuildContext context, String deepLink) onTap,
});
...
class ShareUtility {
  ...
  static Future<void> _copyLink(BuildContext context, String deepLink) async {
    await Clipboard.setData(ClipboardData(text: deepLink));
    Navigator.pop(context);
    HelperUtils.showSnackBarMessage(context, "copied".translate(context));
  }

  static Future<void> _shareLink(BuildContext context, String deepLink) async {
    final box = context.findRenderObject() as RenderBox?;
    final text = "${"shareDetails".translate(context)}:\n$deepLink.";
  ...
  static void share(BuildContext context, DeepLinkTarget target) {
    ...
    final deepLink = target.shareUrl;
            ...
            children: [
              for (final action in _actions)
                ListTile(
                  ...
                  onTap: () => guard.run(() => action.onTap(context, deepLink)),
                ),
            ],
          ),
        );
