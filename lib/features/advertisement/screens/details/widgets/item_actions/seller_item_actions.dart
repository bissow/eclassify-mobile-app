...
class _SellerItemActionsState extends State<SellerItemActions> {
  ...
  bool get showDeleteButton {
    return switch (status) {
      ItemStatus.approved || ItemStatus.softRejected => false,
      _ => true,
    };
  }
  ...
  bool get showResubmitButton => status == ItemStatus.softRejected;
        ...
        if (showResubmitButton)
          Expanded(
            child: _StatusChangeButton(
              ...
              isLoading: _isAnyActionLoading,
              title: 'resubmit'.translate(context),
            ),
          ),
      ],
    );
      ...
      child: AppButton(
        ...
        child: BlocBuilder<ItemStatusCubit, ItemStatusState>(
          builder: (context, state) {
            ...
            return Text(title ?? status.label.translate(context));
          },
        ),
      ),
    );
