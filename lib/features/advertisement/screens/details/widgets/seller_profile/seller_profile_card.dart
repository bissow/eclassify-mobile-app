...
class _SellerProfileCardState extends State<SellerProfileCard> {
  ...
  void _getSeller() {
    if (widget.seller != null) {
      context.read<SellerReviewsCubit>().get(params: widget.seller!.id);
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final result = context.select<SellerReviewsCubit, SellerReviewSummary?>(
      (c) => switch (c.state) {
        DataState<SellerReview>(:final result) =>
          result.metadataAs<SellerReviewSummary>(),
        _ => null,
      },
    );

    final seller = result?.seller;
    final totalRating = result?.totalRating;
    return Padding(
      padding: padding,
            ...
            children: [
              CustomImage(
                ...
                errorImage: UserPlaceholderImage(
                  size: Size.square(70),
                  radius: 16,
                  placeholder: widget.seller?.placeholder,
                ),
              ),
              10.hGap,
              Expanded(
                child: Column(
                  ...
                  children: [
                    if (widget.seller?.isVerified ?? false) VerifiedBadge(),
                    Text(
                      widget.seller?.name ?? 'Anonymous User',
                    ...
                    if ((widget.seller?.email).isNotNullAndNotEmpty)
                      Text(
                        widget.seller!.email,
