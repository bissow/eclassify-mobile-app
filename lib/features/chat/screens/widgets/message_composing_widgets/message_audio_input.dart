            ...
            children: [
              ValueListenableBuilder<bool>(
                ...
                builder: (context, paused, _) => paused
                    ? const Icon(AppIcons.pauseFill, color: Colors.red, size: 18)
...
class _RecordingWavePainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < visible.length; i++) {
      ...
      final amplitudePart = visible[i].clamp(0.0, 1.0) * (size.height - _minHeight);
      final h = (_minHeight + amplitudePart * env).clamp(_minHeight, size.height);
      canvas.drawRRect(
